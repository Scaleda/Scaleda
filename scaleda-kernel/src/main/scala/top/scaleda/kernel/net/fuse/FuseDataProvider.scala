...
import kernel.net.fuse.fs.RemoteFuseGrpc.RemoteFuse
...
/** gRPC data provider for filesystem sync.<br/>
  ...
  */
class FuseDataProvider(sourceRoot: File) extends RemoteFuse {
  ...
  override def read(request: ReadRequest): Future[ReadReply] = async {
    ...
    if (!file.exists()) ReadReply(-ErrorCodes.ENOENT)
      ...
      if (offset > file.length()) logger.warn("read over offset")
      rf.seek(offset)
  ...
  override def write(request: WriteRequest): Future[IntReply] = async {
    ...
    IntReply({
      if (!file.exists()) -ErrorCodes.ENOENT
        ...
        if (offset > file.length()) logger.warn("write over offset")
        rf.seek(offset)
