...
import kernel.net.remote.{RunRequest, StopCommandRequest, StringTriple}
...
class RemoteCommandRunner(
      ...
      val shellThread = new Thread(() => {
        ...
        for (r <- client.run(request)) {
          r.replyType match {
            ...
            case RUN_REPLY_TYPE_COMMAND_ID =>
              KernelLogger.info("recv command id:", r.strValue)
              commandId = Some(r.strValue)
        ...
        () => {
          ...
          while (fsRunning) {
            try {
              ...
              try {
                stream.onNext(
                  FuseTransferMessage.of(0, "login", BinarySerializeHelper.fromAny(remoteCommandDeps.runId))
                )
  ...
  override def canTerminate = commandId.nonEmpty

  private val terminateHandleThread = new Thread(() => {
    while (commandId.isEmpty) Thread.sleep(100)
    KernelLogger.info(s"command id done: ${commandId.get}, will stopCommand")
    Thread.currentThread().setName(s"terminating-remote-${commandId.get}")
    val (client, shutdown) = RemoteClient(remoteCommandDeps.host, port = remoteCommandDeps.port)
    terminating = true
    val reply = client.stopCommand(StopCommandRequest(commandId = commandId.get, timeoutMs = 15000))
    shutdown()
    if (reply.ok) {
      terminated = true
      KernelLogger.warn("Terminate done")
    } else {
      KernelLogger.warn("Cannot terminate remote command:", reply.reason)
    }
  })
  terminateHandleThread.setDaemon(true)

  override def doTerminate(): Unit = {
    terminate = true
    if (!terminateHandleThread.isAlive) terminateHandleThread.start()
  }
}
