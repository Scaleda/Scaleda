...
class MessageListService(val project: Project) extends Disposable with ConsoleReceiver {
  ...
  project.getService(classOf[ConsoleService]).addListener(this, MessageListService.getId)
  ...
  override def getDisplayName: String = ScaledaBundle.message("windows.tool.log.message.title")
}

object MessageListService {
  def getId = "message"
}