  ...
  def handleVivadoProject(project: Project, contentManager: ContentManager): Boolean = {
    ...
    xprFiles.headOption.foreach(xprFile => {
      ...
      try {
        ...
        case e: Throwable => ScaledaIdeaLogger.warn("cannot parse vivado project", e)
      }
      vivadoProject
        .foreach(c => {
              ...
              Toolchain
                ...
                .getOrElse({
                  if (Toolchain.profiles().nonEmpty) {
                    ScaledaIdeaLogger.warn("cannot find vivado path")
                  } else {
          ...
          class ReloadVivadoAction extends AnAction("Refresh Vivado", "refresh", AllIcons.Actions.Refresh) {
            override def actionPerformed(e: AnActionEvent) = {
              try {
                ...
                case e: Throwable => ScaledaIdeaLogger.warn("cannot reload vivado project", e)
              }
            }
          }
