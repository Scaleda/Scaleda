...
class ScaledaRunIdeaHandler(project: Project) extends ScaledaRunHandler {
  ...
  override def onReturn(returnValue: Int, finishedAll: Boolean, meetErrors: Boolean): Unit = OutputLogger(project).info(s"command done, returns $returnValue, finishedAll: $finishedAll, meetErrors: $meetErrors")
}
